# Module 18: Confidential VMs (Preview)

**Top Problems / Features Addressed**
- Data-in-use protection (SEV-SNP/TDX)
...
This module introduces **Confidential VMs (Preview)** in the context of healthcare provider/payor workloads. You will:
...
- Data-in-use protection (SEV-SNP/TDX)
- Attestation workflow
- OS/Driver support matrix
- Performance overhead considerations

## Architecture Diagram – Confidential VMs (Preview)
```mermaid
...
flowchart TB
  subgraph Azure["Azure Subscription"]
    RG["Resource Group"]
    VNET["VNet + Subnets"]
    VM["VM(s): Confidential VMs (Preview)"]
  end
...
sequenceDiagram
  ...
  participant VM as VM: Confidential VMs (Preview)
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 18)
...
3. Apply this module’s configuration:
   ```bash
   bash infra/19_confidential_vm.sh
   ```
