# Module 12: Azure Monitor & Log Analytics

**Top Problems / Features Addressed**
- Collect system logs/metrics
- KQL queries for audit trails

> [!IMPORTANT]
> Use *synthetic* data only. Follow HIPAA/HITRUST: least-privilege RBAC, encrypt data at rest and in transit.

## Intent & Learning Objectives
This module introduces **Azure Monitor & Log Analytics** in the context of healthcare provider/payor workloads. You will:
...
- Collect system logs/metrics
- KQL queries for audit trails
- Alert rules for PHI risks
- Dashboards/workbooks

## Architecture Diagram – Azure Monitor & Log Analytics
```mermaid
...
flowchart TB
  subgraph Azure["Azure Subscription"]
    RG["Resource Group"]
    VNET["VNet + Subnets"]
    VM["VM(s): Azure Monitor & Log Analytics"]
  end
...
sequenceDiagram
  ...
  participant VM as VM: Azure Monitor & Log Analytics
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 12)
...
3. Apply this module’s configuration:
   ```bash
   echo 'Connect VM to Log Analytics Workspace and configure data collection rules (see module steps)'
   ```
