# Module 01: Wide Range of VM Sizes & Series

**Top Problems / Features Addressed**
- Right-size compute (CPU/memory/GPU) for EHR, PACS, analytics
- Performance tuning per workload (latency, throughput)

> [!IMPORTANT]
> Use *synthetic* data only. Follow HIPAA/HITRUST: least-privilege RBAC, encrypt data at rest and in transit.

## Intent & Learning Objectives
This module introduces **Wide Range of VM Sizes & Series** in the context of healthcare provider/payor workloads. You will:
...
- Right-size compute (CPU/memory/GPU) for EHR, PACS, analytics
- Performance tuning per workload (latency, throughput)
- Series selection trade-offs (Dv5, Ev5, Fsv5, NV/H100 GPUs)
- Cost/perf benchmarking approach for healthcare

## Architecture Diagram – Wide Range of VM Sizes & Series
```mermaid
...
flowchart TB
  subgraph Azure["Azure Subscription"]
    RG["Resource Group"]
    VNET["VNet + Subnets"]
    VM["VM(s): Wide Range of VM Sizes & Series"]
  end
...
sequenceDiagram
  ...
  participant VM as VM: Wide Range of VM Sizes & Series
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 01)
