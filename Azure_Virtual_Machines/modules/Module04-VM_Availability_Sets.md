# Module 04: VM Availability Sets

**Top Problems / Features Addressed**
- Increase intra-region availability for patient portals
- Fault/update domain isolation

> [!IMPORTANT]
> Use *synthetic* data only. Follow HIPAA/HITRUST: least-privilege RBAC, encrypt data at rest and in transit.

## Intent & Learning Objectives
This module introduces **VM Availability Sets** in the context of healthcare provider/payor workloads. You will:
...
- Increase intra-region availability for patient portals
- Fault/update domain isolation
- HA testing of web+db tiers
- Maintenance planning and SLAs

## Architecture Diagram – VM Availability Sets
```mermaid
...
sequenceDiagram
  ...
  participant VM as VM: VM Availability Sets
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 04)
...
3. Apply this module’s configuration:
   ```bash
   bash infra/05_availability_set.sh
   ```
