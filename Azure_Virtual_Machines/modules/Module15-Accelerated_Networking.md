# Module 15: Accelerated Networking

**Top Problems / Features Addressed**
- SR-IOV accelerated networking
- Latency/Jitter reduction for imaging

> [!IMPORTANT]
> Use *synthetic* data only. Follow HIPAA/HITRUST: least-privilege RBAC, encrypt data at rest and in transit.

## Intent & Learning Objectives
This module introduces **Accelerated Networking** in the context of healthcare provider/payor workloads. You will:
...
- SR-IOV accelerated networking
- Latency/Jitter reduction for imaging
- NIC constraints and supported SKUs
- Throughput validation

## Architecture Diagram – Accelerated Networking
```mermaid
...
flowchart TB
  subgraph Azure["Azure Subscription"]
    RG["Resource Group"]
    VNET["VNet + Subnets"]
    VM["VM(s): Accelerated Networking"]
  end
...
sequenceDiagram
  ...
  participant VM as VM: Accelerated Networking
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 15)
...
3. Apply this module’s configuration:
   ```bash
   bash infra/15_accelerated_net.sh
   ```
