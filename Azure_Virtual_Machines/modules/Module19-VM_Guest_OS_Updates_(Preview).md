# Module 19: VM Guest OS Updates (Preview)
...
This module introduces **VM Guest OS Updates (Preview)** in the context of healthcare provider/payor workloads. You will:
...
- Automated OS patching
- Maintenance windows
- Reboot coordination (EHR uptime)
- Reporting & compliance evidence

## Architecture Diagram – VM Guest OS Updates (Preview)
```mermaid
...
flowchart TB
  subgraph Azure["Azure Subscription"]
    RG["Resource Group"]
    VNET["VNet + Subnets"]
    VM["VM(s): VM Guest OS Updates (Preview)"]
  end
...
sequenceDiagram
  ...
  participant VM as VM: VM Guest OS Updates (Preview)
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 19)
