# Module 13: Azure Policy & RBAC

**Top Problems / Features Addressed**
- Deny policy for risky SKUs; require tags
- Built-in roles & custom RBAC

> [!IMPORTANT]
> Use *synthetic* data only. Follow HIPAA/HITRUST: least-privilege RBAC, encrypt data at rest and in transit.

## Intent & Learning Objectives
This module introduces **Azure Policy & RBAC** in the context of healthcare provider/payor workloads. You will:
...
## Architecture Diagram – Azure Policy & RBAC
```mermaid
...
sequenceDiagram
  ...
  participant VM as VM: Azure Policy & RBAC
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 13)
...
3. Apply this module’s configuration:
   ```bash
   echo 'Assign built-in policies and set RBAC roles (see infra/13_policy_rbac.sh example)'
   ```
