# Module 14: VM Networking (VNET, NSG, Private Link)

**Top Problems / Features Addressed**
- Isolated VNet & subnets
...
This module introduces **VM Networking (VNET, NSG, Private Link)** in the context of healthcare provider/payor workloads. You will:
...
- Isolated VNet & subnets
- NSG inbound/Outbound rules
- Private Link to PaaS (FHIR, Storage)
- DNS & name resolution planning

## Architecture Diagram – VM Networking (VNET, NSG, Private Link)
```mermaid
...
flowchart TB
  subgraph Azure["Azure Subscription"]
    RG["Resource Group"]
    VNET["VNet + Subnets"]
    VM["VM(s): VM Networking (VNET, NSG, Private Link)"]
  end
...
sequenceDiagram
  ...
  participant VM as VM: VM Networking (VNET, NSG, Private Link)
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 14)
