# Module 03: Azure VM Images & Marketplace

**Top Problems / Features Addressed**
- Rapid deploy of standard images (Ubuntu/Rocky) and healthcare stacks
- Build hardened golden images (CIS baseline)

> [!IMPORTANT]
> Use *synthetic* data only. Follow HIPAA/HITRUST: least-privilege RBAC, encrypt data at rest and in transit.

## Intent & Learning Objectives
This module introduces **Azure VM Images & Marketplace** in the context of healthcare provider/payor workloads. You will:
...
- Marketplace legal/terms considerations for PHI

## Architecture Diagram – Azure VM Images & Marketplace
```mermaid
...
flowchart TB
  subgraph Azure["Azure Subscription"]
    RG["Resource Group"]
    VNET["VNet + Subnets"]
    VM["VM(s): Azure VM Images & Marketplace"]
  end
...
sequenceDiagram
  ...
  participant VM as VM: Azure VM Images & Marketplace
  participant Sec as Defender/Policy
  participant RSV as Recovery Vault
  Eng->>Azure: az scripts (Module 03)
...
3. Apply this module’s configuration:
   ```bash
   bash infra/04_image_marketplace.sh
   ```
