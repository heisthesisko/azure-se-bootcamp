    ...
    **Intent & Learning Objectives:** Logical segmentation within VNets for organizing workloads.
    **Estimated time:** 35–50 min

    **Key Features Demonstrated:**
    - Subnet design (web/app/db/mgmt)
    - Service endpoints & reserved subnets
    - Deploy a test VM

    **Top Problems/Features this module addresses:**
    - Segment workloads (web/app/db/mgmt) for least privilege
    - Prepare subnets required by Azure services
...
> Some resources in this module may accrue cost (e.g., public IPs, App Gateway WAF_v2, Front Door, ExpressRoute, Bastion, vWAN). Use a training subscription and clean up when finished.
    ...
    See: `assets/diagrams/module02_sequence.mmd`
...
2. **Run this module's script**  
   ```bash
   bash infra/m02_subnets.sh
   ```
3. **Validate outcomes** – follow checks below.


    ### Validation
    List subnets and confirm addresses: `az network vnet subnet list -g "$RG_NAME" --vnet-name "$VNET_NAME" -o table`. If VM created, SSH using Bastion later.
