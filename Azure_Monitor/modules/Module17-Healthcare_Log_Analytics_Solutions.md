# Module 17: Healthcare Log Analytics Solutions

**Intent & Learning Objectives:**  
This module explains **Pre-built healthcare analytics (FHIR/HL7/EHR telemetry).** in the context of healthcare (providers and payors). By the end, you will:  
- Understand the core concepts and **three key features** of Healthcare Log Analytics Solutions.  
...
1. **Problem/Feature A:** FHIR/HL7 audit queries and dashboards.  
2. **Problem/Feature B:** EHR interface monitoring patterns.

> [!IMPORTANT]
> Treat all lab data as ePHI. Use only **synthetic** data. Apply least-privilege RBAC on Log Analytics and dashboards.

**Architecture Diagram**  
See: `assets/diagrams/module17_flow.mmd` and `assets/diagrams/module17_sequence.mmd`.

```mermaid
flowchart LR
  A["Module 17: Healthcare Log Analytics Solutions"] --> B["Azure Monitor"]
...
2. **Deploy/Configure**  
   ```bash
   bash infra/17_healthcare_solutions.sh
   ```

3. **Validate in Portal/KQL**  
   Example KQL:
   ```kusto
   AppTraces | where Message contains 'FHIR' | take 20
   ```
...
## Pros, Cons & Insights
**Pros:** Domain-focused insights; jump-starts teams.  
**Cons:** Needs adaptation to each EHR/engine.  
**Insights:** Standardize field names, obfuscate PHI in logs.
