# Module 9: VM Insights

**Intent & Learning Objectives:**  
This module explains **Process maps and dependency insights for VMs hosting PHI/PII.** in the context of healthcare (providers and payors). By the end, you will:  
- Understand the core concepts and **three key features** of VM Insights.  
...
1. **Problem/Feature A:** Process and dependency mapping on VMs.  
2. **Problem/Feature B:** Top processes by CPU/mem with trends.

> [!IMPORTANT]
> Treat all lab data as ePHI. Use only **synthetic** data. Apply least-privilege RBAC on Log Analytics and dashboards.

**Architecture Diagram**  
See: `assets/diagrams/module09_flow.mmd` and `assets/diagrams/module09_sequence.mmd`.

```mermaid
flowchart LR
  A["Module 9: VM Insights"] --> B["Azure Monitor"]
...
2. **Deploy/Configure**  
   ```bash
   bash infra/10_enable_vm_insights.sh
   ```

3. **Validate in Portal/KQL**  
   Example KQL:
   ```kusto
   ServiceMapConnection_CL | take 10
   ```
...
## Pros, Cons & Insights
**Pros:** Great for RCA and zero-trust micro-segmentation.  
**Cons:** Agent overhead; large data on busy servers.  
**Insights:** Use map to derive precise NSG/firewall rules.
