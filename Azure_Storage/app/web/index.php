<?php
// Training-only PHP uploader using SAS (write-only). Do NOT use in production without proper controls.
...
<head>
  <meta charset="utf-8">
  <title>Healthcare Training Uploader</title>
...
<?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])): ?>
  <?php
    ...
    if ($_FILES['file']['size'] > $maxMb * 1024 * 1024) {
      ...
      echo "<p>Server missing BLOB_ACCOUNT or WEB_SAS env vars.</p>";
    } else {
      ...
      if ($code >= 200 && $code < 300) {
        echo "<p>Upload OK: {$name}</p>";
      } else {
        echo "<p>Upload failed ({$code}): " . htmlspecialchars($resp) . "</p>";
      }
    }
  ?>
<?php endif; ?>
...
<blockquote>
<strong>Training note:</strong> Use SAS with minimum permissions (e.g., write and create) and short expiry.
</blockquote>
</body>
</html>
